...
-- version 2021 : rendez vous le 18 mars 2021 : jeudi 
...
-- bascule version 2022 
INSERT INTO rendezvous (vet_id, rv_debut, rv_client)
SELECT vet_id, rv_debut+interval '385 day' as rv_debut, rv_client
FROM rendezvous;	
...
WITH creneauxDisponibles AS 
	(SELECT vet_nom, generate_series('07-04-2022'::date+dis_debut,
						   '07-04-2022'::date+dis_fin-'00:20:00'::time,
						   '20 minutes'::interval) debut
	FROM disponibilite
		INNER JOIN veterinaire
			ON veterinaire.vet_id = disponibilite.vet_id
	WHERE dis_jour = EXTRACT('DOW' FROM '07-04-2022'::date)
	ORDER BY vet_nom, dis_id),
	creneauxReserves AS 
	(SELECT vet_nom, rv_debut debut
	 FROM rendezvous
		...
		WHERE rv_debut 
		BETWEEN '07-04-2022'::date 
		AND '07-04-2022'::date +'23:59:59'::time),
	creneauxRestants AS
